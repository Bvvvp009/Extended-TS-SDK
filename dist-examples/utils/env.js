...
exports.getEnvNumberOptional = getEnvNumberOptional;
exports.getEnvBoolean = getEnvBoolean;
exports.getEnvBooleanOptional = getEnvBooleanOptional;
exports.loadEnv = loadEnv;
exports.getX10EnvConfig = getX10EnvConfig;
/**
 ...
 */
function getEnvNumberOptional(name, defaultValue) {
    const value = process.env[name];
    if (!value) {
        return defaultValue;
    }
 ...
 */
function getEnvBooleanOptional(name, defaultValue) {
    const value = process.env[name];
    if (!value) {
        return defaultValue;
    }
 ...
 */
function getX10EnvConfig(requirePrivateApi = true) {
    loadEnv();
    ...
    if (requirePrivateApi) {
        if (!apiKey)
            throw new Error('API_KEY or X10_API_KEY is not set');
        if (!publicKey)
            throw new Error('Stark_Key_Public or X10_PUBLIC_KEY is not set');
        if (!privateKey)
            throw new Error('Stark_Key_Private or X10_PRIVATE_KEY is not set');
        if (!vaultId)
            throw new Error('Vault_Number or X10_VAULT_ID is not set');
